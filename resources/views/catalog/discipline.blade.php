...
<div class="mx-auto max-w-6xl px-4 py-8">
    ...
    <h1 class="adm-serif mt-2 text-3xl font-semibold text-text">{{ $discipline->name }}</h1>

    <form method="GET" class="mt-4 flex flex-wrap gap-2">
        <input name="q" value="{{ request('q') }}" placeholder="Search courses…" class="rounded-lg border border-border bg-surface px-3 py-2 text-sm">
        <select name="difficulty" class="rounded-lg border border-border bg-surface px-3 py-2 text-sm">
            <option value="">All levels</option>
            @foreach (['beginner','intermediate','advanced','expert'] as $d)
                <option value="{{ $d }}" @selected(request('difficulty') === $d)>{{ ucfirst($d) }}</option>
            @endforeach
        </select>
        <button class="rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-primary-fg">Filter</button>
    </form>
