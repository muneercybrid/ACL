...
<div class="mx-auto max-w-6xl space-y-6">
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
        @php
            ...
            $buckets = [
                ['label'=>'Pending / Invited','count'=>($statusCounts->get('pending') ?? collect())->count() + ($statusCounts->get('invited') ?? collect())->count(),'valueClass'=>'text-warning'],
                ['label'=>'In Progress','count'=>($statusCounts->get('started') ?? collect())->count() + ($statusCounts->get('partially_completed') ?? collect())->count(),'valueClass'=>'text-info'],
                ['label'=>'Awaiting Review','count'=>($statusCounts->get('awaiting_review') ?? collect())->count(),'valueClass'=>'text-warning'],
                ['label'=>'Completed','count'=>($statusCounts->get('completed') ?? collect())->count(),'valueClass'=>'text-success'],
            ];
        @endphp
        @foreach ($buckets as $b)
            <div class="rounded-2xl border border-border bg-surface p-4 shadow-sm">
                <div class="text-xl font-extrabold {{ $b['valueClass'] }}">{{ $b['count'] }}</div>
                ...
                <tbody class="divide-y divide-border">
                    @forelse ($onboardings as $onboarding)
                        <tr class="hover:bg-raised/50 transition">
                            ...
                            <td class="px-6 py-4">
                                @php
                                    $statusChip = $onboarding->isCompleted()
                                        ? 'bg-success-bg text-success'
                                        : ($onboarding->status === 'suspended' ? 'bg-danger-bg text-danger' : 'bg-warning-bg text-warning');
                                    $barClass = $onboarding->isCompleted() ? 'bg-success' : 'bg-primary';
                                @endphp
                                <span class="rounded-full {{ $statusChip }} px-2.5 py-0.5 text-xs font-bold capitalize">{{ str_replace('_', ' ', $onboarding->status) }}</span>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-2">
                                    <div class="h-2 flex-1 rounded-full bg-raised overflow-hidden">
                                        <div class="h-full rounded-full {{ $barClass }}" style="width:{{ $onboarding->getCompletionPercentage() }}%"></div>
                                    </div>
